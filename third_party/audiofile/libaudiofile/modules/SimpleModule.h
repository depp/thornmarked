...
public:
    ...
    virtual void describe() OVERRIDE {
        m_outChunk->f.byteOrder =
            m_inChunk->f.byteOrder == AF_BYTEORDER_BIGENDIAN
    ...
    virtual void run(Chunk &inChunk, Chunk &outChunk) OVERRIDE {
        ...
        case 2:
            run<2, int16_t>(inChunk, outChunk);
            break;
        case 3:
            run<3, char>(inChunk, outChunk);
            break;
        case 4:
            run<4, int32_t>(inChunk, outChunk);
            break;
        case 8:
            run<8, int64_t>(inChunk, outChunk);
            break;
        default:
            assert(false);
            break;
...
private:
    ...
    void run(Chunk &inChunk, Chunk &outChunk) {
        ...
        runSwap<N, T>(reinterpret_cast<const T *>(inChunk.buffer),
                      reinterpret_cast<T *>(outChunk.buffer), sampleCount);
    }
    template <int N, typename T>
...
template <>
inline void SwapModule::runSwap<3, char>(const char *input, char *output,
                                         int count) {
...
struct signConverter {
    ...
    static const int kMinSignedValue = -((1u << kScaleBits) - 1) - 1;

    struct signedToUnsigned
        : public std::unary_function<SignedType, UnsignedType> {
...
public:
    ConvertSign(FormatCode format, bool fromSigned)
        : m_format(format), m_fromSigned(fromSigned) {}
...
private:
    ...
    template <FormatCode Format>
    static void convertSignedToUnsigned(const void *src, void *dst,
                                        size_t count) {
        transform<typename signConverter<Format>::signedToUnsigned>(src, dst,
                                                                    count);
    }
    ...
    template <FormatCode Format>
    static void convertUnsignedToSigned(const void *src, void *dst,
                                        size_t count) {
        transform<typename signConverter<Format>::unsignedToSigned>(src, dst,
                                                                    count);
    }
    ...
    void run(const uint8_t *input, T *output, int sampleCount) {
        for (int i = 0; i < sampleCount; i++) {
            T t =
                ...
                (input[3 * i] << 24) | (input[3 * i + 1] << 16) |
                input[3 * i + 2] << 8;
#else
                (input[3 * i + 2] << 24) | (input[3 * i + 1] << 16) |
                input[3 * i] << 8;
#endif
            output[i] = t >> 8;
        }
...
public:
    ...
    virtual void describe() OVERRIDE { m_outChunk->f.packed = true; }
    virtual void run(Chunk &inChunk, Chunk &outChunk) OVERRIDE {
        int count = inChunk.f.channelCount * inChunk.frameCount;
        if (m_isSigned)
...
struct ConvertIntToFloat : public SimpleModule {
    ...
    virtual void describe() OVERRIDE {
        m_outChunk->f.sampleFormat =
            m_outFormat == kDouble ? AF_SAMPFMT_DOUBLE : AF_SAMPFMT_FLOAT;
    ...
    virtual void run(Chunk &inChunk, Chunk &outChunk) OVERRIDE {
        ...
        if (m_outFormat == kFloat) {
            switch (m_inFormat) {
            case kInt8:
                run<int8_t, float>(src, dst, count);
                break;
            case kInt16:
                run<int16_t, float>(src, dst, count);
                break;
            case kInt24:
            case kInt32:
                run<int32_t, float>(src, dst, count);
                break;
            default:
                assert(false);
            }
            ...
            switch (m_inFormat) {
            case kInt8:
                run<int8_t, double>(src, dst, count);
                break;
            case kInt16:
                run<int16_t, double>(src, dst, count);
                break;
            case kInt24:
            case kInt32:
                run<int32_t, double>(src, dst, count);
                break;
            default:
                assert(false);
            }
...
struct ConvertInt : public SimpleModule {
    ...
    virtual void describe() OVERRIDE {
        getDefaultPCMMapping(m_outChunk->f.sampleWidth, m_outChunk->f.pcm.slope,
...
struct Clip : public SimpleModule {
    Clip(FormatCode format, const PCMInfo &outputMapping)
        : m_format(format), m_outputMapping(outputMapping) {}
    virtual const char *name() const OVERRIDE { return "clip"; }
    virtual void describe() OVERRIDE { m_outChunk->f.pcm = m_outputMapping; }
    virtual void run(Chunk &inChunk, Chunk &outChunk) OVERRIDE {
        const void *src = inChunk.buffer;
        void *dst = outChunk.buffer;
        int count = inChunk.frameCount * inChunk.f.channelCount;
        ...
        switch (m_format) {
        case kInt8:
            run<int8_t>(src, dst, count);
            break;
        case kInt16:
            run<int16_t>(src, dst, count);
            break;
        case kInt24:
        case kInt32:
            run<int32_t>(src, dst, count);
            break;
        case kFloat:
            run<float>(src, dst, count);
            break;
        case kDouble:
            run<double>(src, dst, count);
            break;
        default:
            assert(false);
        }
...
struct ConvertFloatToIntClip : public SimpleModule {
    ConvertFloatToIntClip(FormatCode inputFormat, FormatCode outputFormat,
        ...
        assert(m_outputFormat == kInt8 || m_outputFormat == kInt16 ||
               m_outputFormat == kInt24 || m_outputFormat == kInt32);
    }
    ...
    virtual void run(Chunk &inChunk, Chunk &outChunk) OVERRIDE {
        ...
        if (m_inputFormat == kFloat) {
            ...
            case kInt8:
                run<float, int8_t>(src, dst, count);
                break;
            case kInt16:
                run<float, int16_t>(src, dst, count);
                break;
            case kInt24:
            case kInt32:
                run<float, int32_t>(src, dst, count);
                break;
            default:
                assert(false);
            }
            ...
            switch (m_outputFormat) {
            case kInt8:
                run<double, int8_t>(src, dst, count);
                break;
            case kInt16:
                run<double, int16_t>(src, dst, count);
                break;
            case kInt24:
            case kInt32:
                run<double, int32_t>(src, dst, count);
                break;
            default:
                assert(false);
            }
...
public:
    ApplyChannelMatrix(FormatCode format, bool isReading, int inChannels,
                       int outChannels, double minClip, double maxClip,
                       const double *matrix);
...
public:
    Transform(FormatCode format, const PCMInfo &inputMapping,
              const PCMInfo &outputMapping)
        : m_format(format),
          m_inputMapping(inputMapping),
          m_outputMapping(outputMapping) {
